...
@st.cache_data
def load_fundamentals_file(path="fundamentals.csv"):
    try:
        f = pd.read_csv(path)
        ...
        return None, "fundamentals.csv not found in the repository."
...
with st.sidebar:
    ...
    st.info(
        "Fundamentals are loaded automatically from fundamentals.csv in this repository. "
        "Update that file in GitHub when you want to refresh the database."
    )
    st.warning("Yahoo Finance data can be delayed/rate-limited. This is not a live NSE feed.")

st.subheader("📁 Fundamental database")
fund_df, fund_error = load_fundamentals_file("fundamentals.csv")

if fund_error:
    st.warning(
        f"{fund_error} Add a file named fundamentals.csv beside setup2.py "
        "to activate the full fundamental score."
    )
elif fund_df is not None:
    st.success(
        f"Loaded permanent fundamentals database: {len(fund_df):,} symbols. "
        "You do not need to upload it each time."
    )
...
if r is not None and not r.empty:
    ...
    # Native Streamlit chart — no Plotly dependency required.
    chart_df = d.set_index("Date")[["Close", "EMA20", "EMA50", "SMA200", "BOP"]].copy()
    st.line_chart(chart_df, height=520)

    st.write(
        f"**Levels:** Stop ₹{rec["Stop"]:.2f} • T1 ₹{rec["T1"]:.2f} • "
        f"T2 ₹{rec["T2"]:.2f}"
    )
